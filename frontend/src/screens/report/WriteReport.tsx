...
import { selectUserID, userState } from "../../recoil/atoms";
...
const WriteReport = () => {
  ...
  const writer = headOffice
    ? headOffice.find((member) => member[0] === user.username) || null
    : null;
  const writerRD = RDOffice
    ? RDOffice.find((member) => member[0] === user.username) || null
    : null;
  ...
  const updateApprovalLines = (report: string) => {
      ...
      return [
        {
          name: "참조",
          checked: true,
          selectedMembers: vacationFixed ? [vacationFixed] : [],
        },
        { name: "대표이사", checked: true, selectedMember: approvalFixed },
        {
          name: "관리부서장",
    ...
    switch (report) {
      ...
      case "휴가신청서":
        newApprovalLines =
          user.company === "본사" ? vacationLines() : RDWriteLines();
        break;
      case "기획서":
        newApprovalLines =
          user.company === "본사" ? ManagementLines() : RDWriteLines();
        break;
      case "중간보고서":
        newApprovalLines =
          user.company === "본사" ? ManagementLines() : RDWriteLines();
      ...
      case "자기개발비 신청서":
        newApprovalLines =
          user.company === "본사" ? supportTeamLast() : RDWriteLines();
        break;
      case "법인카드 신청서":
        newApprovalLines =
          user.company === "본사" ? supportTeamLast() : RDWriteLines();
  ...
  const renderPages = () => {
    ...
    for (let i = 1; i <= numPages; i++) {
      pages.push(<Page key={`page_${i}`} pageNumber={i} width={1000} />);
    }
    return pages;
  };
                              ...
                              {line.checked ? (
                                line.selectedMember ? (
                                  ...
                                  <div className="approvals_contents">
                                    {line.selectedMembers.map(
                                      (member, index) => (
                                        <div
                                          key={index}
                                          className="approval_small_name"
                                        >
          ...
          <div className="write_btm_container2">
            {file ? (
              ...
              >
                ...
                <div className="upload-text-btm">
                  클릭 후 파일 선택이나 드래그로 파일 첨부 가능합니다.
                </div>
              </div>
            )}
          </div>
